    ...
    if (req.query.product_id !== undefined) {
      ...
      redis.get(styleKey, async function (err, reply) {
        if (reply !== null) {
          ...
          const results = []
          const style = await pool.query(`SELECT id as style_id, name, original_price, sale_price, default_style as "default?" FROM style WHERE id_product = ${req.query.product_id}`)

          for (let i = 0; i < style.rows.length; i++) {
            const newObj = { ...style.rows[i] }
            const styleId = style.rows[i].style_id
            // add the photos for the style
            const photo = await pool.query(`SELECT thumbnail_url, url FROM style LEFT JOIN photo ON photo.id_style=style.id WHERE id_style = ${styleId}`)
            const photoArray = []
            ...
            const sku = await pool.query(`SELECT sku.id, sku.quantity as quantity, sku.size as size FROM style LEFT JOIN sku ON sku.id_style=style.id WHERE id_style = ${styleId}`)
            newObj.skus = {}
            const skuObj = {}
    ...
    if (req.query.product_id !== undefined) {
      ...
      redis.get(styleKey, async function (err, reply) {
        if (reply !== null) {
          ...
          const results = []
          const style = await pool.query(`SELECT id as style_id, name, original_price, sale_price, default_style as "default?" FROM style WHERE id_product = ${randProduct}`)
    ...
    if (req.query.product_id !== undefined) {
      ...
      redis.get(relatedKey, async function (err, reply) {
        if (reply !== null) {
          ...
          const related = await pool.query(`SELECT related_products.related_product_id FROM product LEFT JOIN related_products ON related_products.current_product_id=product.id WHERE product.id=${randProduct}`)
          const relatedIDs = []
          for (let i = 0; i < related.rows.length; i++) {
            relatedIDs.push(related.rows[i].related_product_id)
          }
